CREATE DEFINER=`root`@`%` PROCEDURE `insertFull`(ignore_existing_order INT(1), id_center INT, id INT, order_date DATE, id_order_type INT, id_adv INT, client_name VARCHAR(255), bdate DATE, id_user INT, zone CHAR(2), fee DECIMAL(10,2), id_stage INT, article VARCHAR(255), id_informer INT, id_act CHAR(40), act_no INT, act_date DATE, cycle INT, act_reg_date DATE, payment_date DATE, outgoings DECIMAL(8,2), bank_reg_date DATE, id_checker INT, outg_post DECIMAL(8, 2), outg_daynight DECIMAL(8,2))
BEGIN
