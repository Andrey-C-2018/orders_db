...
class XWindow : public XEventHandler{
	...
	bool is_created;
	void OnWindowCreationCompleted() override { is_created = true; }
	XWindow *GetThisAsXWindow() override { return this; }
...
public:
	...
	XWindow(XWindow &&obj) noexcept;
	XWindow &operator=(const XWindow &obj) = delete;
	XWindow &operator=(XWindow &&obj) noexcept;
...
void XWindow::CreateImpl(XWindow *parent, const Tchar *class_name, \
	...
	is_created = false;
	_plHWND hwnd;
	hwnd = creation_func(parent ? parent->GetInternalHandle() : nullptr, class_name, \
