...
void CVerticalSizer::addWidget(XWidget *widget, const Tchar *label) {
	...
	widget->Create(parent, FL_WINDOW_CLIPSIBLINGS | FL_WINDOW_BORDERED | FL_WINDOW_VISIBLE, \
					label, x, y, width - margin_left - margin_right, row_height);

	y += row_height + GAP;
}

void CVerticalSizer::initNestedSizer(CSizer &sizer) const {

	setParametersTo(sizer, x, y, width - margin_left - margin_right, \
					height + y_initial - y - margin_bottom);
}

void CVerticalSizer::addNestedSizerByActualSize(const CSizer &sizer) {

	assert(!sizer.isNull());
	int actual_height = sizer.getActualHeight();
	y += actual_height + GAP;
}
